...
getRadio, getRadioActive, getCheckBoxes, genCheckBoxes, setCheckBox,
...
waction
  :: (FormInput view, Monad m,MonadIO m) =>
     View view m a
     -> (a -> FlowM view m b)
     -> View view m b
waction formt act= View $ do
   ...
   case mx of
    Just x -> do
      ...
      case br of
       GoBack -> do
               modify (\s -> s{validated= False}) !> "GOBACK"
               return $ FormElm form Nothing
       NoBack r    ->  return . FormElm form $ Just r !> "NOBACK"
       BackPoint r ->  return . FormElm form $ Just r !> "BACKPOINT" -- bad. no backpoints
...
mFlowState0 = MFlowState 0 False [] True  False  "en" [] False False (error "token of mFlowState0 used") 0 0 [] [] stdHeader False [] M.empty
