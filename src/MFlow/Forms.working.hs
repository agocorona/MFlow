...
backPointReturn x= BackT . return $ BackPoint x         -- !> "backpointreturn"
...
cachedWidget key t mf = View $ StateT $ \s -> do
        ...
        let(FormElm form2 mx2, s'') = execute $  runStateT  (runView mf) $  s
        let s'''=  s''{ mfSequence= mfSequence s' }
...
-- | example of user\/password only form (no validation) to be used with 'userWidget'
...
--       -> WState view m (Maybe String)
   ...
   val _ _ = return $ Just "Please fill in the fields to login or register"
...
ask x = do
     ...
     FormElm forms mx <- lift $ runView $ widget x        --  !> "ask"
     st' <- get
--     liftIO $ print "before case"                                      !> ("after1 "++show ( mfSequence st'))
     case mx of  
       Just x -> do
       ...
       _ ->
         if mfGoingBack st && not (formMatch (mfSequence st) (mfSequence st') (mfEnv st'))     -- !> "test1"
          then do
             put st'{mfSequence= head1 (prevSeq st')
                    ,mfGoingBack= True
                    ,prevSeq=tail1 (prevSeq st')}
             fail repeatPlease                                !> "repeatPlease 1"
          else do
             ...
             st'' <- get
             if  not $ formMatch (mfSequence st) (mfSequence st') (mfEnv st'')  -- !> "test2"
              then do
                 put st''{mfSequence= head1 (prevSeq st'')
                        ,mfGoingBack= True
                        ,prevSeq=tail1 (prevSeq st'')}
                 fail repeatPlease                                       !> "repeatPlease 2"
              else ask x
    where
    head1 [] = 0
    head1 xs= head xs
    ...
    formMatch s s' env
       | s ==  s' = True
       | otherwise=  (not . null) (filter (isJust . getparm env) [s .. s'-1])       -- !> "---" !> show s  !> show s' !> show env
...
instance (Monoid view,Widget a b m view) => Widget [a] b m view where
  widget xs = View $ do
      forms <- mapM proc xs
      ...
      where
      proc x= do
        FormElm form mx <- runView $ widget x
        return $ FormElm [mconcat form ] mx

-- | concat a list of widgets of the same type
wconcat :: (MonadIO m,Monoid view, Functor m)=> [View view m a]  -> View view m a
