...
module Menu (Options(..), askm, pagem, mainMenu,stdheader) where
...
askm w= ask $ do
   filename <- getSessionData `onNothing` error "source filename not set"
   (El.div ! At.style (attr "background-color:#EEEEEE;float:left\
                 \;width:30%\
