{-# OPTIONS -F -pgmFtrhsx  -XDeriveDataTypeable
...
hello :: Env -> IO String
hello env =  return  "hello, this is an stateless response"
...
instance FormLet Prod IO Html where
   digest mp=  wrap (\c -> <table> <% c %> </table>) $
      Prod <$> wrap (\c -> <tr> <td> enter the name  </td> <td> <% c $> </td> </tr>)
                    (getString (pname <$> mp))
           <*> wrap (\c -> <tr> <td> enter the price </td> <td> <% c %> </tr>)
                    (getInt ( pprice <$> mp))))
...
shopProds cart products=
  <br>
  `prepend`                    -- add Html to the first widget
  p << "-----Shopping List-----"
  `prepend`
  widget(Selection{
  ...
  `mix`                    -- operator to mix two wdigets
