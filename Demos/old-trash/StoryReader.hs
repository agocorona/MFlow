...
UserStoryConf= UserContext{ name :: String
                          , byMail :: Maybe String
                          , hasConfirmed :: Bool
                          , seek::: Integer}

data UserContext= map String UserStoryConf

userstoryconf0= UserContext "" Nothing False 0
...
showStories   = do
   ...
   startcontext= M.fromList [  (n,userstoryconf0{name=n}) | n <- M.keys stor]
   showStories1 startcontext
   where

   showStories1  usercontext =  do
     story <- step $  ask  $  userFormOrName **>  listStories (M.keys usercontext)
     userstorycontext= case lookup story usercontext of
                           Nothing -> userstorycontext0
                           Just c ->  c

     newc  <- step $ navigate userstorycontext
     M.insert  usercontext story newc
     showStories1  usercontext
     where
       navigate context= do
               (chunk,seekit,size)  <- getChunk
                ...
                case seek of
                    ...
                    -2000 -> byMail context
                    seek  -> do
                      navigate   context{seek=seek} story
...
byMail context= do
  ...
  message <- showBuffer seekit size chunk

  sendSimpleEmail ( SMTPBackend server hostname)
         "noreply@"++hostname
         mail message

  ask $ thespan << "The mail has been sent to you" ++> wlink "" "click here"
  return context{byMail= Just mail, confirmed= False}
